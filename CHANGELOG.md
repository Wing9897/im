...
- Wipe-only SQLite baseline **stamp 43** (`SCHEMA_SEMVER` `0.1.0-beta.44`): `emoji` on `analysis_tasks`／`user_events`／`recurring_schedules`; drop `ui_prefs` `schedule_emojis`／`task_emojis`. `GET /api/v1/calendar/window` is SoT for time-window reads (Timeline／Board display and notify scan). Intelligence/map keep `GET /api/v1/results/events`. User-events／recurring CRUD stay for 我的日程 editors. Prior stamps ≤42 hard-reject → `uv run python scripts/reset_local_databases.py --apply`. Stamps 27–42 are absorbed into the [SCHEMA-BASELINE](docs/SCHEMA-BASELINE.md) floor.
