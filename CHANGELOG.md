...
- **正式版基线：** 产品版本 `1.0.0`（离开 beta）。全新 git 历史起点；`main` push → `quality` → `version`（from tags；无 tag 时用 `VERSION` 原样）→ 三平台 `package` → `release`（打 `v*` + GitHub Release）。不 bot 回写 `VERSION` 到 `main`。
