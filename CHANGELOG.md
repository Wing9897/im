...
- Wipe-only SQLite baseline **stamp 42** (`SCHEMA_SEMVER` `0.1.0-beta.43`): no DDL columns dropped. Forces a local reset so leftover stamp-41 `ui_prefs` blobs are discarded. Prior stamps ≤41 hard-reject → `uv run python scripts/reset_local_databases.py --apply`. Stamps 27–41 are absorbed into the [SCHEMA-BASELINE](docs/SCHEMA-BASELINE.md) floor.
- SPA notifications workspace page is `NotifyWorkspacePage` (`web/src/pages/notify/`; route `/notify`; tabs `types`／`notify`／`history`). HTTP `/api/v1/actions*` outbound automation unchanged. Unique `TimelineCalendarRendering` assertions moved into CalendarView／GanttView／ControlBar tests; that file is deleted. Shared calendar fetch/cache renamed to `fetchSharedCalendarOccurrences`.
