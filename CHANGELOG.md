...
- Schema stamp **4** (`SCHEMA_SEMVER` `1.3.0`) stores calendar-share publish state in `calendar_share_publish` (unpublished worksets have no row). Stamp-1/2/3 databases backup once then walk `SCHEMA_MIGRATIONS` to 4. Empty DBs create current DDL and stamp 4.
- Schema stamp **3** (`SCHEMA_SEMVER` `1.2.0`) adds `worksets.emoji` and `worksets.description`. Stamp-1/2 databases backup once then walk `SCHEMA_MIGRATIONS` to 3. Empty DBs create current DDL and stamp 3.
