...
- Wipe-only SQLite baseline **stamp 28** (`SCHEMA_SEMVER` `0.1.0-beta.29`): drop `items.expires_at`／`remind_before_days` (derive-on-read from linked `kind=expires`); timeline source `item` → `item_remind`; non-current stamps hard-reject → explicit reset
