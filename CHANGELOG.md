...
- Wipe-only SQLite baseline **stamp 43** (`SCHEMA_SEMVER` `0.1.0-beta.44`): `emoji` on `analysis_tasks`／`user_events`／`recurring_schedules`; drop `ui_prefs` `schedule_emojis`／`task_emojis`. Timeline and board calendar surfaces SoT is `GET /api/v1/calendar/window` (board events-list + items remind summary included). Intelligence page and board map keep `GET /api/v1/results/events`. Prior stamps ≤42 hard-reject → `uv run python scripts/reset_local_databases.py --apply`. Stamps 27–42 are absorbed into the [SCHEMA-BASELINE](docs/SCHEMA-BASELINE.md) floor.
- SPA notifications workspace page is `NotifyWorkspacePage` (`web/src/pages/notify/`; route `/notify`; tabs `types`／`notify`／`history`). HTTP `/api/v1/actions*` outbound automation unchanged. Unique `TimelineCalendarRendering` assertions moved into CalendarView／GanttView／ControlBar tests; that file is deleted. Dual-path `sharedCalendarFetch` (`/results/events` + `/calendar/occurrences`) is gone from Timeline/Board display — those surfaces use `GET /api/v1/calendar/window`. Notify still reads `/calendar/occurrences` + timed `/results/events`.
