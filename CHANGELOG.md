...
- Subscriptions live on `/subscriptions` (我的訂閱 / 搜尋訂閱). IntelligenceCalendar `subscriptions` is the source of truth; IM GET/POST/DELETE `/api/v1/calendar-share/subscriptions` proxy IC `/me/subscriptions` (DELETE unsubscribes on the server). Timeline events follow that same list. There is no local `calendar_share_subscriptions` cache — IC 502 fails closed. Closed or grant-revoked calendars drop from 我的訂閱 when IC GET prunes that subscriber’s own row. Timeline still splits the source filter into side-by-side 本機 (`Layers`) / 訂閱 (`Bookmark`) columns (catalog add/remove is not in that dialog). Search uses calendar-server `GET /search` via IM `GET /api/v1/calendar-share/search`.
