...
Intelligence Monitor 是給**單一家庭**在自己電腦上用的情報工作站：把 Telegram、Discord、RSS、MQTT、Email 等來源收進來，整理成可篩的情報與事件，再落到日曆、任務、物品與地圖。不是多租戶雲端服務。

- **個人情報過濾** — 情報事件可依工作集、分析任務、關鍵字與時段篩選，並切列表／卡片／地圖；地圖可疊加實時資訊與事件、也可只看其中一種，並用 LIVE 時間窗。助手能在同一批本機情報裡查問。
- **多任務管理與輸出** — 「任務設定」是獨立導航（`/tasks`），任務掛在工作集下；情報、排行榜、專案經理、週期任務可分開跑。結果寫入情報事件與排行榜；時間規劃看月曆與甘特。對外是可選的工作集日曆分享／發佈（[https://subscribe.devents.tech](https://subscribe.devents.tech)），不是一鍵匯出報告。
- **本機家庭工作集** — 資料預設留在本機 SQLite；用工作集把情報、日程與物品歸在同一家。AI 供應商與金鑰由你自己接。
- **助手隨時叫出** — `Ctrl+J`（macOS `⌘+J`）開文字輸入；未開時可用「閃現」氣泡看語音／快捷回覆。完整對話走側欄「助手」。
