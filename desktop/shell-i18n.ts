...
const SHELL_COPY: Record<ShellLocale, ShellCopy> = {
  'zh-Hant': {
    ...
    schemaHardRejectHint:
      '本機資料庫結構不相容（schema baseline 3／schemaSemver 0.1.0-beta.3；舊庫須重置）。\n\n請到設定 → 資料 重置，或執行：\nscripts/reset_local_databases.py --apply\n\n然後重新啟動應用程式。',
  ...
  'zh-Hans': {
    ...
    schemaHardRejectHint:
      '本地数据库结构不兼容（schema baseline 3／schemaSemver 0.1.0-beta.3；旧库须重置）。\n\n请到设置 → 数据 重置，或运行：\nscripts/reset_local_databases.py --apply\n\n然后重新启动应用程序。',
  ...
  en: {
    ...
    schemaHardRejectHint:
      'The local database schema is incompatible (schema baseline 3 / schemaSemver 0.1.0-beta.3; older databases must be reset).\n\nReset with Settings → Data, or run:\nscripts/reset_local_databases.py --apply\n\nThen restart the app.',
