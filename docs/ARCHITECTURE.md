...
| `db/` | SQLite persistence via aiosqlite — current baseline **v7** (`SCHEMA_SEMVER` `1.6.0`) DDL split under `db/schema_domains/` and aggregated by `db/schema.py` (fingerprint in `db/schema_fingerprint.py`), bootstrap／reject in `db/schema_bootstrap.py` plus additive runner in `db/schema_migrate.py` and production steps in `db/schema_steps.py`（`SCHEMA_FLOOR` 7＝current；`SCHEMA_MIGRATIONS` empty; stamp-1…6 files hard-reject → backup then reset; future stamps hard-reject → update the app; never silent wipe）, connection/reset wrapper in `db/database.py`. Retired monolithic `schema_ddl.py` is gone. |
...
- **Assistant / AI**: `/assistant` stays conversational. AI settings live at `/settings/ai/*` (`provider` / `voice` / `staff`; `web/src/pages/settings/ai/`). There are no `/ai/*` routes — leftover `/ai/provider|voice|staff`, `/ai`, and `/ai/analysis-strategy` fall through to `NotFoundPage`. Sessions `GET/PUT /api/v1/ui-prefs/assistant/sessions`. Contracts: [`assistant.md`](agent/assistant.md)、[`a2a.md`](agent/a2a.md)、[`mcp.md`](agent/mcp.md)、[`agent.md`](agent/agent.md) (tick UI `/tasks/:taskId/agent`; legacy `/project` retired, no redirect).
- **Worksets / tasks**: `/worksets` is the catalog (`catalog` / `graph` only). Exact `/tasks` is the independent analysis-task list (`DashboardViewer`). Leftover `/worksets?tab=tasks` stays on the catalog — no redirect to `/tasks`. Editors stay `/tasks/new` and `/tasks/:taskId/edit|agent`. Workset detail `/worksets/:id` keeps a scoped member-task list. HTTP `/api/v1/tasks*` is unchanged. Simple mode hides `/tasks` (list + editors); leftover `?tab=tasks` bookmarks still stay on `/worksets`.
...
| Timeline source filter | `pages/timeline/components/TimelineSourceFilterDialog` | Timeline toolbar: same tree **plus** a subscribe column (`components/subscriptions/SubscribeFilterGroup`); do not fold into `SourceFilterDialog` |
...
- Shared timed-event projectors: `domain/timeline/timedEventMerge`. Board / intelligence source filter UI: `components/SourceFilterDialog` (`{ taskIds, worksetIds } | null`). Timeline uses `TimelineSourceFilterDialog` (tree + subscribe column). Board RRULE / `item_remind` filter aligns with Timeline (`worksetId` ownership). Do **not** merge the three persistence paths:

| Surface | Dialog | Persistence |
|---------|--------|-------------|
| Timeline | `TimelineSourceFilterDialog` | Device LS `im:timeline:selected-sources` (tree) + sibling `im:timeline:subscribe-filter` (subscribe keys; not `SourceFilterSelection`) |
| Intelligence | `SourceFilterDialog` | Device LS `im:intelligence:selected-sources` |
| Board widgets | `SourceFilterDialog` | Server `ui-prefs` `widgetState.sourceFilters` (`BoardSourceFilterPref`; `GET/PUT /api/v1/ui-prefs/board`) |
...
| `analysis_tasks` | Shared task definitions (`leaderboard` / `intel_event` / `agent`), trigger-purpose `schedule_rrule` for AI timers, agent policy columns (`trigger_mode` / caps / outputs)<sup>19</sup>, and AI scheduling overrides; contains no recurring calendar payload columns (standalone series live on `recurring_schedules`) |
...
| `items` | Trackable inventory (optional `emoji`; optional `quantity`／`unit`; free-form details in `notes`); ownership via `workset_id`. **No item-level `price` or `attributes_json`** — purchase／effective amounts live on linked `user_events` with `kind=purchase_effective` (`amount` + `direction`). **Items date model:** linked `user_events` with `kind=expires` are the source of truth (title presets 到期／Expires are UX only). Cache columns `expires_at`／`remind_before_days` are gone<sup>28</sup>; wire `expiresAt`／`remindBeforeDays` on ItemResponse are **derived on read** from the primary linked expires event (non-dismissed, earliest `created_at`). No write-through sync — expiry authority is `user_events.kind=expires` (`server/calendar/user_event_kinds.py` + items derive-on-read queries). There is no `purchased_at` column. Calendar `source=item_remind` projects remind DATE only as floating all-day (`itemDateKind=remind`; occurrence ids still `item:{id}:remind`) |

Current stamp / floor / support matrix: [`SCHEMA-BASELINE.md`](SCHEMA-BASELINE.md). Historical column notes above (not current requirements): <sup>19</sup> agent policy columns on `analysis_tasks`; <sup>28</sup> item expiry cache drop (derive-on-read remains); <sup>30</sup> `user_events.origin=mcp`.
...
Four layers plus two snapshot helpers — do not merge them.
...
Schema lifecycle split: `test_schema_floor.py` (stamp-7 floor＝current / `1.6.0`, stamp 1–6 reject, retired future stamps 27/45 hard-reject, lookalike reject, lifespan log); `test_schema_migrate.py` (injected test-only `3→10` walk / mid-fail / backup; production `SCHEMA_MIGRATIONS` is empty); `test_db_schema.py` (fingerprint / unstamped current / newer-than-supported / lookalikes). Shared fixtures: `schema_fixtures.py`.
