...
| `web_search/` | Provider clients + `WebSearchExecutionService` + constrained `web.fetch`. Vocabulary／secret columns／wire names SoT: `domain/web_search_providers.py` (DDL CHECK, Pydantic mixins, serializers, FE `assistantWebSearchRoute.ts` drift-tested). Keyed vendors are one `KeyedSearchSpec` table in `providers.py` (no parallel HTTP POST copies). Outbound SSRF is `outbound.validate_outbound_url` for search, fetch, and LLM. Agent ticks via AgentRuntime; count / master-switch as params |
...
7. **Packaging:** First-class Desktop delivery is **Windows NSIS**, **macOS DMG/zip**, and **Linux AppImage/deb** (`desktop/electron-builder.yml`). **CLI** is the same headless server entry as `python -m server`／`intelligence-monitor`, shipped as a per-OS PyInstaller zip (`npm run package:cli` → `dist/cli/intelligence-monitor-cli-<os>-<arch>.zip`) from the Desktop sidecar onedir. The PyInstaller binary must be built on the **target OS** (no cross-compile). PRs and `main` pushes run Ubuntu `quality` + build only (`.github/workflows/ci.yml`). Publishing is `.github/workflows/release.yml`: a `v*` tag push (or Release `workflow_dispatch`, which **pushes the tag first**) injects SemVer into the build workspace only — **no** bot commit to main — then runs the `package` matrix (win／mac／linux: `dist:*` + `verify:desktop:full` [= `desktop_verify` only; desktop vitest stays in `quality`] + `package:cli`), creates the GitHub Release (Desktop **and** CLI; missing assets fail), and publishes GHCR. **Product version authority is git tags**; repo `VERSION` may lag. Schema stamp／`SCHEMA_SEMVER` are DB-contract identities and need not equal the product tag.

**Headless container (GHCR):** `Dockerfile` ships the FastAPI server + built SPA (no Electron). Data volume `/data`; see `docker-compose.yml` and `npm run docker:build`. GHCR push follows the Release workflow (`v*` tag or that workflow's `workflow_dispatch`); `main` pushes never publish an image. Dockerfile `HEALTHCHECK` + CI deploy smoke cover post-publish readiness.
