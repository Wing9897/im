...
| `db/` | SQLite persistence via aiosqlite — current baseline **v5** (`SCHEMA_SEMVER` `1.4.0`) DDL split under `db/schema_domains/` and aggregated by `db/schema.py` (fingerprint in `db/schema_fingerprint.py`), bootstrap／reject in `db/schema_bootstrap.py` plus additive runner in `db/schema_migrate.py` and production steps in `db/schema_steps.py`（`SCHEMA_FLOOR` 5＝current；`SCHEMA_MIGRATIONS` empty; stamp-1/2/3/4 files hard-reject → backup then reset; future stamps hard-reject → update the app; never silent wipe）, connection/reset wrapper in `db/database.py`. Retired monolithic `schema_ddl.py` is gone. |
...
| Timeline source filter | `pages/timeline/components/TimelineSourceFilterDialog` | Timeline toolbar: same tree **plus** a subscribe column (`SubscribeFilterGroup`); do not fold into `SourceFilterDialog` |
...
Schema lifecycle split: `test_schema_floor.py` (stamp-5 floor＝current, stamp 1–4 reject, retired future stamps 27/45 hard-reject, lookalike reject, lifespan log); `test_schema_migrate.py` (injected test-only `3→10` walk / mid-fail / backup; leftover listing remap; production `SCHEMA_MIGRATIONS` is empty); `test_db_schema.py` (fingerprint / unstamped current / newer-than-supported / lookalikes). Shared fixtures: `schema_fixtures.py`.
