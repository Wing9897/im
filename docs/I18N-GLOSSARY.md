...
| 層 | 定稿用語 | 代碼／路徑（勿改） | 說明 |
|----|----------|-------------------|------|
| 花名冊頁 | AI 員工介紹 | 路由 `/settings/ai/staff`（舊 `/ai/staff` 永久轉址）；i18n `settings:staff.*`／`nav`·`common` 的 `aiStaff`；`web/src/domain/aiStaff/` | 只讀介紹頁；以 `AI_STAFF_ROSTER` 為準（前線：助手、任務顧問；後勤：排行榜分析員、情報任務分析員、專案經理）+ 頁內「客戶經理」（code id `liaison`，非 `AiStaffId` runtime）。**勿**在產品文案寫死「六位／Six AI」等易過時人數 |
