...
**Current stamp is 43** (`SCHEMA_SEMVER` = `0.1.0-beta.44`). Startup creates the authoritative DDL only for an empty database, stamps an exact-current unstamped structure, and accepts an exact stamp-43 fingerprint. Every other non-empty schema hard-rejects before collector/scheduler startup with `python scripts/reset_local_databases.py --apply` in the error. Startup never migrates, backs up, restores, or silently deletes a database. Public identity is returned by `GET /api/v1/health` as `schemaVersion` and `schemaSemver`; `PRAGMA user_version` remains the integer stamp.
...
|------------------------|---------|
| **43** (current, exact fingerprint) | Full runtime (`schemaSemver` = `0.1.0-beta.44`) |
| **42** and earlier (prior) | Hard-reject → reset |
...
There is no migration registry, `_data_migrations` ledger, schema-upgrade route/UI, backup marker, or post-migration validator in stamp 43. `test_schema_wipe_floor.py` guards this hard cut and the reset guidance.

**Stamp 43 is the wipe-only floor** (`SCHEMA_SEMVER` `0.1.0-beta.44`; prior stamps ≤42 hard-reject). This stamp adds `emoji TEXT DEFAULT NULL` on `analysis_tasks`, `user_events`, and `recurring_schedules` (items already had `emoji`) and drops leftover `ui_prefs` keys `schedule_emojis` / `task_emojis`. **Stamps 27–42 are absorbed into this floor** — do not keep a per-stamp migration narrative here. A future in-place migration must be introduced deliberately as a new contract; no dormant fake migration chain remains.

## Schema v43 explicit reset
...
$backup = Join-Path ([Environment]::GetFolderPath("Desktop")) ("IntelligenceMonitor-pre-v43-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
Summary of the stamp-43 wipe-only behavior above:

| Opened database | Startup behavior | Mutation |
|-----------------|------------------|---------|
| Empty, version 0 | Create v43 DDL, validate its full fingerprint, then stamp 43 | Schema creation and v43 stamp |
| Unstamped current, version 0 | Require the exact v43 fingerprint and stamp 43 | Stamp only |
| Current, version 43 | Validate the exact v43 fingerprint on every startup | None |
...
There is no `MigrationStep` registry or content-migration ledger on stamp 43. A future in-place migration must be introduced as an explicit new contract.
