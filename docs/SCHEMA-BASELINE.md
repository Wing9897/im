...
**Current stamp is 40** (`SCHEMA_SEMVER` = `0.1.0-beta.41`). Startup creates the authoritative DDL only for an empty database, stamps an exact-current unstamped structure, and accepts an exact stamp-40 fingerprint. Every other non-empty schema hard-rejects before collector/scheduler startup with `python scripts/reset_local_databases.py --apply` in the error. Startup never migrates, backs up, restores, or silently deletes a database. Public identity is returned by `GET /api/v1/health` as `schemaVersion` and `schemaSemver`; `PRAGMA user_version` remains the integer stamp.
...
|------------------------|---------|
| **40** (current, exact fingerprint) | Full runtime (`schemaSemver` = `0.1.0-beta.41`) |
| **39** and earlier (prior) | Hard-reject → reset |
...
There is no migration registry, `_data_migrations` ledger, schema-upgrade route/UI, backup marker, or post-migration validator in stamp 40. `test_schema_wipe_floor.py` guards this hard cut and the reset guidance.

**Stamp 40 is the wipe-only floor** (`SCHEMA_SEMVER` `0.1.0-beta.41`; prior stamps ≤39 hard-reject). Prior stamp history is absorbed into this floor — see [`CHANGELOG.md`](../CHANGELOG.md) / prior release notes; do not keep a per-stamp migration narrative here. A future in-place migration must be introduced deliberately as a new contract; no dormant fake migration chain remains.

## Schema v40 explicit reset
...
$backup = Join-Path ([Environment]::GetFolderPath("Desktop")) ("IntelligenceMonitor-pre-v40-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
Summary of the stamp-40 wipe-only behavior above:

| Opened database | Startup behavior | Mutation |
|-----------------|------------------|---------|
| Empty, version 0 | Create v40 DDL, validate its full fingerprint, then stamp 40 | Schema creation and v40 stamp |
| Unstamped current, version 0 | Require the exact v40 fingerprint and stamp 40 | Stamp only |
| Current, version 40 | Validate the exact v40 fingerprint on every startup | None |
...
There is no `MigrationStep` registry or content-migration ledger on stamp 40. A future in-place migration must be introduced as an explicit new contract.
