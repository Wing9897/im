...
**Stamp 7 is both the schema floor and current** (`SCHEMA_FLOOR` = 7, `CURRENT_SCHEMA_VERSION` = 7, `SCHEMA_SEMVER` = `1.6.0`). Fresh databases create current DDL only (including `schema_meta`, `worksets.description` / `cover_data_url`, and `calendar_share_publish` with `last_cover`). Stamp 7 stores calendar-share publish state in `calendar_share_publish` (unpublished worksets have no row) and does not keep `system_config.calendar_share_worksets` JSON or leftover `calendar_share_subscriptions`. Household auto-sync lives **only** in `system_config` (`calendar_share_auto_sync` / `calendar_share_auto_sync_interval_seconds`); the stamp-6 mirrored cache columns on `calendar_share_publish` are gone, as is the startup remap of legacy listing aliases (`off` / `details` / `busy`) and the legacy read-only slug regex. There is no lineage from retired pre-cut stamps (former 27/45 / `0.1.0-beta.*`). Startup creates the authoritative DDL only for an empty database, stamps an exact-current unstamped structure, and accepts an exact stamp-7 fingerprint.
...
|------------------------|---------|
| **7** (floor and current, exact fingerprint) | Full runtime (`schemaSemver` = `1.6.0`) |
| **6** / **5** / **4** / **3** / **2** / **1** | Hard reject — backup, then reset (no additive walk) |
...
Production `SCHEMA_MIGRATIONS` is empty (`SCHEMA_FLOOR` = `CURRENT` = 7). There is no JSON `calendar_share_worksets` migration, `_data_migrations` ledger, schema-upgrade route/UI, `down()`, or startup data remap. `test_schema_floor.py` hard-rejects stamp 1–6, retired stamps 27/45, and lookalike current-stamp files missing `schema_meta`. `test_schema_migrate.py` proves the injected test-only chain (3→10).

Do not revive retired stamps 27/45 as a migration chain. **This stamp-7 Release does not upgrade old databases** (no 6→7 walk). The next real DDL change appends stamp **8**: leave `SCHEMA_FLOOR` at 7, set `CURRENT` to 8, and register a non-empty `SCHEMA_MIGRATIONS` additive step. Later upgrades continue as stamp 9+ on that same walk, not as another wipe.

## Schema v7 explicit reset

There is no automatic deletion. Future stamps are not converted. Stamp 1–6 files are not upgraded — back them up, then reset. Before resetting a **corrupt**, **below-floor**, or **future** database, stop Electron, `npm run dev`, and any standalone server so SQLite WAL state is closed. If data must be retained for manual recovery, copy the database outside every Intelligence Monitor data directory first.
...
The helper deletes only known SQLite database files and their `-wal`／`-shm` sidecars. It deliberately leaves backups, Telegram sessions, `secret.key`, `connection.json`, directories, and volumes untouched. Restart creates a fresh v7 database (schema_meta is seeded by DDL; no demo-data auto-seed). Restoring an old stamped database does not upgrade a **future** stamp—it restores the original unsupported state. Restoring a stamp 1–6 file still rejects until you reset.

## Schema support matrix

| Opened database | Startup behavior | Mutation |
|-----------------|------------------|---------|
| Empty, version 0 | Create current DDL (including seeded `schema_meta`), validate its full fingerprint, then stamp 7 | Schema creation and v7 stamp |
...
| Stamp 6 / 5 / 4 / 3 / 2 / 1 | Hard-reject: backup then reset | None |
