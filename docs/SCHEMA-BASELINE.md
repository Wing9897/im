...
**Current stamp is 42** (`SCHEMA_SEMVER` = `0.1.0-beta.43`). Startup creates the authoritative DDL only for an empty database, stamps an exact-current unstamped structure, and accepts an exact stamp-42 fingerprint. Every other non-empty schema hard-rejects before collector/scheduler startup with `python scripts/reset_local_databases.py --apply` in the error. Startup never migrates, backs up, restores, or silently deletes a database. Public identity is returned by `GET /api/v1/health` as `schemaVersion` and `schemaSemver`; `PRAGMA user_version` remains the integer stamp.
...
|------------------------|---------|
| **42** (current, exact fingerprint) | Full runtime (`schemaSemver` = `0.1.0-beta.43`) |
| **41** and earlier (prior) | Hard-reject → reset |
...
There is no migration registry, `_data_migrations` ledger, schema-upgrade route/UI, backup marker, or post-migration validator in stamp 42. `test_schema_wipe_floor.py` guards this hard cut and the reset guidance.

**Stamp 42 is the wipe-only floor** (`SCHEMA_SEMVER` `0.1.0-beta.43`; prior stamps ≤41 hard-reject). No DDL columns were dropped; this stamp forces a local reset so leftover `ui_prefs` blobs from stamp-41 databases are discarded. **Stamps 27–41 are absorbed into this floor** — do not keep a per-stamp migration narrative here. A future in-place migration must be introduced deliberately as a new contract; no dormant fake migration chain remains.

## Schema v42 explicit reset
...
$backup = Join-Path ([Environment]::GetFolderPath("Desktop")) ("IntelligenceMonitor-pre-v42-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
Summary of the stamp-42 wipe-only behavior above:

| Opened database | Startup behavior | Mutation |
|-----------------|------------------|---------|
| Empty, version 0 | Create v42 DDL, validate its full fingerprint, then stamp 42 | Schema creation and v42 stamp |
| Unstamped current, version 0 | Require the exact v42 fingerprint and stamp 42 | Stamp only |
| Current, version 42 | Validate the exact v42 fingerprint on every startup | None |
...
There is no `MigrationStep` registry or content-migration ledger on stamp 42. A future in-place migration must be introduced as an explicit new contract.
