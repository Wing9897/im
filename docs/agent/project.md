...
- Schema：stamp v8／`schemaSemver` 0.1.0-beta.9（wipe-only；非當前 stamp 須 reset；AI 排程 `schedule_rrule` trigger-only；`user_events.origin` 含 `project`／`ics`；物品域 `items`／`item_categories`）— 見 [`ARCHITECTURE.md`](../ARCHITECTURE.md)
