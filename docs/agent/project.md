...
- Schema：stamp v4／`schemaSemver` 0.1.0-beta.6（wipe-floor：empty `SCHEMA_MIGRATIONS`；舊 stamp 須 reset；`user_events.origin` 含 `project`／`ics`）— 見 [`ARCHITECTURE.md`](../ARCHITECTURE.md)
