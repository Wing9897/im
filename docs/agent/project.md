...
- Schema：stamp v7／`schemaSemver` 0.1.0-beta.8（wipe-only；非當前 stamp 須 reset；`user_events.origin` 含 `project`／`ics`；物品域 `items`／`item_categories`）— 見 [`ARCHITECTURE.md`](../ARCHITECTURE.md)
