...
- Schema：stamp v6／`schemaSemver` 0.1.0-beta.7（wipe-only；非當前 stamp 須 reset；`user_events.origin` 含 `project`／`ics`；物品域 `items`／`item_categories`）— 見 [`ARCHITECTURE.md`](../ARCHITECTURE.md)
