...
First-release rule (``--from-tags`` only):
  ...
  so the inaugural tag matches the committed baseline (e.g. ``1.0.0`` → ``v1.0.0``).
  Subsequent runs bump from the latest tag.
...
def resolve_next(*, from_tags: bool) -> tuple[str, str, str]:
    """Return ``(display_base, next_version, source)``.

    ``source`` is ``tag``, ``file-first`` (no tags yet — VERSION as-is), or ``file``.
    """
    if from_tags:
        tagged = latest_tag_version()
        if tagged is not None:
            return tagged, bump(tagged), "tag"
        current = read_version_file()
        return current, current, "file-first"
    current = read_version_file()
    return current, bump(current), "file"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--from-tags",
        action="store_true",
        help="Base version on latest git tag v* (fallback: VERSION file as-is when no tags)",
    )
    parser.add_argument(
        "--print-only",
        action="store_true",
    ...
    try:
        current, next_version, source = resolve_next(from_tags=args.from_tags)
