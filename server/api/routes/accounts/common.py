"""Platform-agnostic account endpoints (list/delete/reconnect/refresh).

Shared account helpers live in ``helpers.py``; platform modules import them
directly. This module owns the shared FastAPI router and generic account routes.
"""
...
router = APIRouter(prefix="/api/v1/accounts", tags=["accounts"], dependencies=API_DEPS)
...
def _platform_info_fns() -> dict[str, _PlatformInfoFn]:
    ...
    from server.api.routes.accounts.discord import discord_bot_info_batch
    from server.api.routes.accounts.email import email_mailbox_info
    from server.api.routes.accounts.feeds import mqtt_broker_info, rss_feed_info
    from server.api.routes.accounts.http import http_source_info
    ...
    async def _rss_info(db: Any, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        channels_by_account = await list_account_channels_batch(db, [str(row["id"]) for row in rows], "rss")
        return [await rss_feed_info(db, row, channels=channels_by_account.get(str(row["id"]), [])) for row in rows]

    async def _http_info(db: Any, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        channels_by_account = await list_account_channels_batch(db, [str(row["id"]) for row in rows], "http")
        return [await http_source_info(db, row, channels=channels_by_account.get(str(row["id"]), [])) for row in rows]

    async def _email_info(db: Any, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        channels_by_account = await list_account_channels_batch(db, [str(row["id"]) for row in rows], "email")
        return [email_mailbox_info(row, channels_by_account.get(str(row["id"]), [])) for row in rows]
    ...
    if set(_PLATFORM_INFO_FNS) != COLLECTOR_PLATFORMS_WITH_LIST_ENRICHMENT:
        raise RuntimeError(
            "accounts list enrichment map must match "
...
async def list_accounts(request: Request) -> list[dict]:
    """Return Account[] for all accounts.
    ...
    if "platform" in request.query_params:
        raise http_error(
            400,
            "Query parameter 'platform' is not supported; use GET /accounts/{platform}",
            error_code=VALIDATION_ERROR,
        )
    return await _list_accounts_for_platform(request, None)


async def _list_accounts_for_platform(request: Request, platform: str | None) -> list[dict]:
    """Internal helper for typed platform list handlers (and unfiltered GET /accounts)."""
    db = get_db(request)
    ...
    if info_fn is not None:
        ...
        rows = await accounts_queries.fetch_account_rows_for_platform(db, platform, include_credentials=True)
        return await info_fn(db, rows)
    if platform:
        rows = await accounts_queries.fetch_account_rows_for_platform(db, platform, include_credentials=False)
    else:
        rows = await accounts_queries.fetch_all_account_list_rows(db)
    return [serialize_account(row) for row in rows]


@router.get("/telegram", response_model=list[AccountResponse])
async def list_telegram_accounts(request: Request) -> list[dict]:
    return await _list_accounts_for_platform(request, "telegram")


@router.get("/discord", response_model=list[DiscordBotInfoResponse])
async def list_discord_accounts(request: Request) -> list[dict]:
    return await _list_accounts_for_platform(request, "discord")


@router.get("/rss", response_model=list[RssFeedInfoResponse])
async def list_rss_accounts(request: Request) -> list[dict]:
    return await _list_accounts_for_platform(request, "rss")


@router.get("/http", response_model=list[HttpSourceInfoResponse])
async def list_http_accounts(request: Request) -> list[dict]:
    return await _list_accounts_for_platform(request, "http")


@router.get("/mqtt", response_model=list[MqttBrokerInfoResponse])
async def list_mqtt_accounts(request: Request) -> list[dict]:
    return await _list_accounts_for_platform(request, "mqtt")


@router.get("/email", response_model=list[EmailMailboxInfoResponse])
async def list_email_accounts(request: Request) -> list[dict]:
    return await _list_accounts_for_platform(request, "email")


@router.delete("/{account_id}", status_code=204)
async def delete_account(request: Request, account_id: str) -> Response:
    db = get_db(request)
    await get_account_row(db, account_id)
    collector = get_collector(request)
    if collector is not None:
        await collector.stop_adapter(account_id)
    await accounts_queries.delete_account(db, account_id)
    return Response(status_code=204)


@router.post(
    "/{account_id}/reconnect",
    response_model=AddAccountResponse,
    response_model_exclude_unset=True,
)
async def reconnect_account(request: Request, account_id: str) -> dict:
    db = get_db(request)
    await get_account_row(db, account_id)
    collector = get_collector(request)
    ...
    try:
        result = await collector.reconnect_account(account_id)
    ...
    return add_account_response(row, next_step)


@router.post("/refresh-all", response_model=RefreshAllAccountsResponse)
async def refresh_all_accounts(request: Request) -> dict:
    db = get_db(request)
    collector = get_collector(request)
    rows = await accounts_queries.fetch_all_account_rows(db)
    ...
    async def _resolve_status(row: dict[str, Any]) -> tuple[str, str]:
        ...
        if collector is not None:
            try:
                check = await collector.check_connection(account_id)
                ...
                logger.warning("refresh-all check failed for %s: %s", account_id, exc)
                status = "error"
    ...
    for account_id, status in resolved:
        if status == "connected":
            ...
            errors.append(account_id)
        else:
            verification_required.append(account_id)
    ...
    return {
        ...
        "verificationRequiredAccountIds": verification_required,
        "errorAccountIds": errors,
    }
