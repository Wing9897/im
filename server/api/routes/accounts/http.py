"""HTTP poll source accounts: create, update, and list."""
...
class HttpSourcePatchBody(BaseModel):
...
async def http_source_info(
    ...
    *,
    ...
    if channels is None:
        channels = await list_account_channels(db, account_id, "http")
...
async def create_http_source(request: Request, body: HttpSourceBody) -> dict:
    ...
    account_id, _result, error = await connect_source_account(
        request,
        platform="http",
    ...
    if error is not None:
        return await _http_response(db, account_id, status="error", error_message=error)

    async def _finalize() -> dict[str, Any]:
        await bind_account_channel(db, account_id, "http", credentials["url"], credentials["url"])
        await mark_account_connected(db, account_id, name=body.name or credentials["url"])
        return await _http_response(db, account_id, status="connected", error_message=None)

    return await finalize_source_account(request, account_id, _finalize)


@router.patch("/http/{account_id}", response_model=AddHttpSourceResponse)
async def update_http_source(request: Request, account_id: str, body: HttpSourcePatchBody) -> dict:
    db = get_db(request)
    row = await get_account_row(db, account_id)
    if str(row.get("platform")) != "http":
        raise http_error(400, "Account is not an HTTP source", error_code=VALIDATION_ERROR)
    ...
    async def _after_success(db: Any, account_id: str, _result: Any) -> None:
        await bind_account_channel(db, account_id, "http", merged["url"], merged["url"])
