...
async def setup_register(request: Request, body: RegisterBody) -> DeviceSessionTokensResponse:
    ...
    return DeviceSessionTokensResponse.model_validate(tokens)


@router.post("/login", response_model=DeviceSessionTokensResponse)
async def setup_login(request: Request, body: LoginBody) -> DeviceSessionTokensResponse:
    db = get_db(request)
...
@router.post("/change-password", dependencies=API_DEPS, response_model=SetupOkResponse)
async def setup_change_password(request: Request, body: ChangePasswordBody) -> SetupOkResponse:
    db = get_db(request)
    if not await is_bootstrapped(db):
        raise http_error(
            409,
    ...
    return SetupOkResponse(ok=True)


@router.post("/reset-password", response_model=SetupOkResponse)
async def setup_reset_password(request: Request, body: ResetPasswordBody) -> SetupOkResponse:
...
async def setup_refresh(request: Request, body: RefreshBody) -> DeviceSessionTokensResponse:
    db = get_db(request)
...
@router.post("/logout", dependencies=API_DEPS, response_model=SetupOkResponse)
async def setup_logout(request: Request) -> SetupOkResponse:
    db = get_db(request)
...
@router.get("/devices", dependencies=API_DEPS, response_model=DeviceListResponse)
async def setup_list_devices(request: Request) -> DeviceListResponse:
    db = get_db(request)
    current = await resolve_session_id_for_access_token(db, presented_token(request))
    return DeviceListResponse.model_validate({"devices": await list_devices(db, current_session_id=current)})


@router.delete("/devices/{device_id}", dependencies=API_DEPS, response_model=SetupOkResponse)
async def setup_revoke_device(request: Request, device_id: str) -> SetupOkResponse:
    db = get_db(request)
