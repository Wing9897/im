...
async def list_templates() -> list[TaskTemplateResponse]:
    return [TaskTemplateResponse.model_validate(preset) for preset in BUILTIN_PRESETS]


@router.get("/activity-spans", response_model=list[TaskActivitySpanResponse])
async def activity_spans(request: Request) -> list[TaskActivitySpanResponse]:
    rows = await fetch_activity_span_rows(get_db(request))
    return [TaskActivitySpanResponse.model_validate(serialize_activity_span(row)) for row in rows]
...
async def list_tasks(
    request: Request,
    analysis_mode: str | None = qalias("analysisMode", default=None),
    workset_id: str | None = qalias("worksetId", default=None),
) -> list[TaskResponse]:
    try:
        payloads = await list_tasks_payload(
            get_db(request),
    ...
    return [TaskResponse.model_validate(payload) for payload in payloads]


@router.post("", status_code=201, response_model=TaskResponse)
async def create_task(request: Request, body: TaskConfigBody) -> TaskResponse:
    try:
