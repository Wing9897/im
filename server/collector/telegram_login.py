...
the manager's adapter registry but only serve the accounts routes during an
...
async def _ensure_telegram_adapter(
    ...
    adapter = registry.adapters.get(account_id)
    ...
    registry.adapters[account_id] = adapter
    return adapter, True
...
async def start_telegram_login(
    ...
    adapter, created = await _ensure_telegram_adapter(registry, account_id, api_id, api_hash)
    try:
        ...
        try:
            ...
            if created and registry.adapters.get(account_id) is adapter:
                del registry.adapters[account_id]
        raise
