...
in ``{sessions_dir}/{source_id}.session.txt`` instead.
...
def string_session_path(session_dir: str, source_id: str) -> Path:
    return Path(session_dir) / f"{source_id}.session.txt"


def persist_string_session_token(session_dir: str, source_id: str, token: str) -> Path:
    """Write the StringSession token; raises ``ValueError`` when *token* is empty."""
    cleaned = (token or "").strip()
    if not cleaned:
        raise ValueError(f"Refusing to persist empty Telegram session for source {source_id}")
    Path(session_dir).mkdir(parents=True, exist_ok=True)
    path = string_session_path(session_dir, source_id)
    path.write_text(cleaned, encoding="utf-8")
    return path


def load_string_session(session_dir: str, source_id: str) -> StringSession:
    """Load a persisted string session token; empty session when absent."""
    Path(session_dir).mkdir(parents=True, exist_ok=True)

    token_file = string_session_path(session_dir, source_id)
