...
TASK_CONFIG_SCHEMA_PROMPT = (
    ...
    "- promptTemplate: the LLM prompt template for analyzing messages (string)\n"
    f"- analysisMode: one of {_ANALYSIS_MODE_PROMPT_VALUES} (string)\n"
    "- analysisTimeRange: one of '1d', '7d', '30d', 'all' (string)\n"
    "- scheduleType / scheduleValue: only when the user asks to change schedule\n"
    "- includeInTimeline: bool; only for analysisMode=event; default true "
