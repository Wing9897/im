...
``_run_one_wave`` (in ``agent_tick_wave``) runs one AgentRuntime chat page;
``drain_agent_waves`` loops until the backlog is empty or pause / disable /
...
class DrainOutcome:
    ...
    cursor: AgentMessageCursor | str | None = None


async def drain_agent_waves(
    *,
    db: Database,
    ...
    while messages:
        ...
        await update_agent_batch_message_count(db, batch_id, outcome.total_messages)
    ...
    if not outcome.drained:
        ...
        remaining = await fetch_agent_messages_since(
            db,
            task_id=task_id,
            since=outcome.cursor,
            limit=1,
