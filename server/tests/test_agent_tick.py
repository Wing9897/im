"""Agent tick registration, empty-skip, and drain waves (no live LLM)."""
...
async def _insert_agent_task(
    ...
    *,
    agent_wave_interval_seconds: int | None = 0,
) -> None:
    now = utc_now_iso()
...
def test_build_agent_tick_seed_includes_prompt_and_empty_messages() -> None:
    seed = build_agent_tick_seed(
...
def test_build_agent_base_prompt_pins_goals() -> None:
...
@pytest.mark.asyncio
async def test_agent_task_is_registered_on_scheduler(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-reg")
...
async def test_execute_agent_tick_skips_llm_without_messages(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-run")
...
async def test_execute_agent_tick_persists_tool_calls(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-tools")
...
async def test_execute_agent_tick_drains_multiple_waves(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-drain", agent_wave_interval_seconds=0)
...
async def test_execute_agent_tick_cools_between_waves(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-cool", agent_wave_interval_seconds=7)
    await _bind_source_with_messages(
        db,
        task_id="proj-cool",
        count=_MESSAGE_SUMMARY_LIMIT + 1,
...
async def test_execute_agent_tick_reuses_session_across_waves(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-session", agent_wave_interval_seconds=0)
    await _bind_source_with_messages(
        db,
...
async def test_execute_agent_tick_stops_before_next_wave_when_paused(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-pause", agent_wave_interval_seconds=0)
    await _bind_source_with_messages(
        db,
        task_id="proj-pause",
        count=_MESSAGE_SUMMARY_LIMIT + 1,
...
async def test_execute_agent_tick_stops_before_next_wave_when_task_disabled(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-disable", agent_wave_interval_seconds=0)
    await _bind_source_with_messages(
        db,
...
async def test_execute_agent_tick_wave_hard_timeout_defers_remaining(app) -> None:
    db: Database = app.state.db
    await _insert_agent_task(db, "proj-timeout", agent_wave_interval_seconds=0)
    await _bind_source_with_messages(
        db,
    ...
    with (
        ...
        patch("server.scheduler.agent_tick_wave.get_config_int", side_effect=_config_int),
    ):
