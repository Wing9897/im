...
def test_retry_handle_cleanup_is_identity_safe(account_id: str, old_completes_first: bool) -> None:
    ...
    owner._retry_tasks = {account_id: replacement_task}
    ...
    for completed in callbacks:
        owner._remove_retry(account_id, completed)
        if completed is old_task and replacement_task not in callbacks[: callbacks.index(completed)]:
            assert owner._retry_tasks[account_id] is replacement_task

    assert account_id not in owner._retry_tasks
