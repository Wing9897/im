"""Contract keys: accounts & sources routes."""

from __future__ import annotations

import json
from types import SimpleNamespace

from server.secrets import MASKED_SECRET, unprotect_text
from server.tests import seed
from server.tests.contract_helpers import assert_keys

HTTP_SOURCE_INFO_KEYS = [
    "account",
    "channel",
    "url",
    "method",
    "authType",
    "bearerToken",
    "basicUsername",
    "basicPassword",
    "headers",
    "bodyType",
    "body",
    "pollIntervalSeconds",
    "maxContentChars",
    "timeoutSeconds",
    "lastError",
    "lastSuccessAt",
]


class _StubCollector:
    """Minimal collector mock for create/reconnect contract tests."""

    def __init__(
        self,
        *,
        reconnect_result: dict | None = None,
        reconnect_error: str | None = None,
    ) -> None:
        self.adapters: dict[str, object] = {}
        self._reconnect_result = reconnect_result
        self._reconnect_error = reconnect_error

    async def reconnect_account(self, account_id: str) -> dict:
        if self._reconnect_result is not None:
            if self._reconnect_error is not None:
                self.adapters[account_id] = SimpleNamespace(state=SimpleNamespace(last_error=self._reconnect_error))
            return self._reconnect_result
        return {"connected": True, "status": "connected"}

    async def create_discord_bot(self, _host: object, account_id: str, bot_token: str) -> dict:
        return {
            "channels": [
                {"id": "30001", "name": "general", "guild_name": "Test Guild"},
            ],
        }

    async def update_discord_bot(self, _host: object, account_id: str, credentials: dict) -> dict:
        return await self.create_discord_bot(_host, account_id, str(credentials.get("bot_token") or ""))

    async def create_rss_feed(
        self,
        _host: object,
        account_id: str,
        feed_url: str,
        poll_interval_seconds: int = 300,
    ) -> dict:
        return {"feed_title": "New Feed"}

    async def update_rss_feed(self, _host: object, account_id: str, credentials: dict) -> dict:
        return {"feed_title": "Patched Feed"}

    async def create_http_source(self, _host: object, account_id: str, credentials: dict) -> dict:
        return {"status": "connected"}

    async def update_http_source(self, _host: object, account_id: str, credentials: dict) -> dict:
        return {"status": "connected"}

    async def create_mqtt_broker(
        self,
        _host: object,
        account_id: str,
        broker_url: str,
        topics: list[str],
        username: str | None = None,
        password: str | None = None,
        client_id: str | None = None,
    ) -> dict:
        return {}

    async def update_mqtt_broker(self, _host: object, account_id: str, credentials: dict) -> dict:
        return {"status": "connected"}

    def install_source_stubs(self, monkeypatch) -> None:
        """Route account create/update through this stub via manager_sources."""
        for name in (
            "create_discord_bot",
            "update_discord_bot",
            "create_rss_feed",
            "update_rss_feed",
            "create_http_source",
            "update_http_source",
            "create_mqtt_broker",
            "update_mqtt_broker",
        ):
            monkeypatch.setattr(f"server.collector.manager_sources.{name}", getattr(self, name))


def _stub_collector(app, monkeypatch, **kwargs) -> _StubCollector:
    stub = _StubCollector(**kwargs)
    stub.install_source_stubs(monkeypatch)
    app.state.collector = stub
    return stub


async def test_list_accounts_default(client):
    resp = await client.get("/api/v1/accounts")
    assert resp.status_code == 200
    body = resp.json()
    assert len(body) == 5
    for account in body:
        assert_keys(
            account,
            ["id", "platform", "name", "status", "lastError", "updatedAt"],
            "GET /accounts item",
        )


def test_account_list_openapi_uses_concrete_item_models(app):
    schema = app.openapi()
    expected = {
        "/api/v1/accounts": "AccountResponse",
        "/api/v1/accounts/telegram": "AccountResponse",
        "/api/v1/accounts/discord": "DiscordBotInfoResponse",
        "/api/v1/accounts/rss": "RssFeedInfoResponse",
        "/api/v1/accounts/http": "HttpSourceInfoResponse",
        "/api/v1/accounts/mqtt": "MqttBrokerInfoResponse",
        "/api/v1/accounts/email": "EmailMailboxInfoResponse",
    }
    for path, model_name in expected.items():
        response_schema = schema["paths"][path]["get"]["responses"]["200"]["content"]["application/json"]["schema"]
        assert response_schema["type"] == "array", path
        assert response_schema["items"]["$ref"] == f"#/components/schemas/{model_name}", path


async def test_list_accounts_platform_query_rejected(client):
    resp = await client.get("/api/v1/accounts", params={"platform": "telegram"})
    assert resp.status_code == 400


async def test_list_accounts_telegram_has_name(client):
    resp = await client.get("/api/v1/accounts/telegram")
    body = resp.json()
    assert len(body) == 1
    account = body[0]
    assert account["name"] == "+886912345678"


async def test_list_accounts_discord_is_botinfo(client):
    resp = await client.get("/api/v1/accounts/discord")
    body = resp.json()
    assert len(body) == 1
    bot = body[0]
    assert_keys(bot, ["account", "channels"], "DiscordBotInfo")
    assert_keys(
        bot["account"],
        ["id", "status", "name"],
        "DiscordBotInfo.account",
    )
    assert isinstance(bot["channels"], list) and bot["channels"]
    for channel in bot["channels"]:
        assert_keys(
            channel,
            ["platformChannelId", "name", "guildName"],
            "DiscordChannelInfo",
        )


async def test_list_accounts_rss_is_feedinfo(client):
    resp = await client.get("/api/v1/accounts/rss")
    body = resp.json()
    assert len(body) == 1
    feed = body[0]
    assert_keys(
        feed,
        ["account", "feedUrl", "pollIntervalSeconds", "channel", "lastError", "lastSuccessAt"],
        "RssFeedInfo",
    )
    assert feed["feedUrl"] == seed.RSS_CHANNEL[1]
    assert feed["channel"] is not None
    assert_keys(
        feed["channel"],
        ["id", "platform", "platformId", "channelName"],
        "RssFeedInfo.channel",
    )
    assert_keys(
        feed["account"],
        ["id", "status", "name", "lastError", "updatedAt"],
        "RssFeedInfo.account",
    )


async def test_list_accounts_mqtt_is_brokerinfo(client):
    resp = await client.get("/api/v1/accounts/mqtt")
    body = resp.json()
    assert len(body) == 1
    broker = body[0]
    assert_keys(
        broker,
        ["account", "brokerUrl", "topics", "lastError", "lastSuccessAt"],
        "MqttBrokerInfo",
    )
    assert broker["brokerUrl"] == seed.MQTT_CHANNEL[1]
    assert broker["topics"] == ["news/#", "alerts/hk"]
    assert_keys(
        broker["account"],
        ["id", "status", "name"],
        "MqttBrokerInfo.account",
    )


async def test_refresh_all(client):
    resp = await client.post("/api/v1/accounts/refresh-all")
    assert resp.status_code == 200
    assert_keys(
        resp.json(),
        [
            "totalAccounts",
            "connectedCount",
            "verificationRequiredCount",
            "errorCount",
            "verificationRequiredAccountIds",
            "errorAccountIds",
        ],
        "refresh-all",
    )


async def test_reconnect_account_response_and_db_status(client, app):
    db = app.state.db
    await db.execute(
        "UPDATE accounts SET status = 'error', last_error = 'stale error' WHERE id = ?",
        (seed.DISCORD_ACCOUNT,),
    )
    app.state.collector = _StubCollector(reconnect_result={"connected": True, "status": "connected"})

    resp = await client.post(f"/api/v1/accounts/{seed.DISCORD_ACCOUNT}/reconnect")
    assert resp.status_code == 200
    body = resp.json()
    assert_keys(body, ["nextStep", "account"], "AddAccountResponse")
    assert body["nextStep"] == "connected"
    assert body["account"]["status"] == "connected"
    assert body["account"]["lastError"] is None

    row = await db.fetch_one("SELECT status, last_error FROM accounts WHERE id = ?", (seed.DISCORD_ACCOUNT,))
    assert row["status"] == "connected"
    assert row["last_error"] is None


async def test_reconnect_account_failure_persists_error(client, app):
    db = app.state.db
    app.state.collector = _StubCollector(
        reconnect_result={"connected": False, "status": "error"},
        reconnect_error="token revoked",
    )

    resp = await client.post(f"/api/v1/accounts/{seed.RSS_ACCOUNT}/reconnect")
    assert resp.status_code == 200
    body = resp.json()
    assert body["nextStep"] == "error"
    assert body["account"]["status"] == "error"
    assert body["account"]["lastError"] == "token revoked"

    row = await db.fetch_one("SELECT status, last_error FROM accounts WHERE id = ?", (seed.RSS_ACCOUNT,))
    assert row["status"] == "error"
    assert row["last_error"] == "token revoked"


async def test_create_discord_bot_response_shape(client, app, monkeypatch):
    _stub_collector(app, monkeypatch)
    resp = await client.post(
        "/api/v1/accounts/discord",
        json={"botToken": "discord-test-token"},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert_keys(body, ["account", "channels", "status", "errorMessage"], "AddDiscordBotResponse")
    assert body["status"] == "connected"
    assert body["channels"]
    for channel in body["channels"]:
        assert_keys(channel, ["id", "name", "platformChannelId", "guildName"], "DiscordChannelInfo")


async def test_create_rss_feed_response_shape(client, app, monkeypatch):
    _stub_collector(app, monkeypatch)
    feed_url = "https://example.com/new-feed.xml"
    resp = await client.post(
        "/api/v1/accounts/rss",
        json={"feedUrl": feed_url, "name": "New Feed"},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert_keys(body, ["account", "channel", "feedTitle", "status", "errorMessage"], "AddRssFeedResponse")
    assert body["status"] == "connected"
    assert body["feedTitle"] == "New Feed"
    assert body["channel"] is not None

    stored = await app.state.db.fetch_value(
        "SELECT credentials FROM accounts WHERE id = ?",
        (body["account"]["id"],),
    )
    creds = json.loads(unprotect_text(stored))
    assert creds["poll_interval_seconds"] == 300


async def test_http_source_create_list_and_patch_contract(client, app, monkeypatch):
    _stub_collector(app, monkeypatch)
    created = await client.post(
        "/api/v1/accounts/http",
        json={
            "url": "https://example.com/api/events",
            "name": "Events API",
            "authType": "bearer",
            "bearerToken": "http-secret-token",
            "headers": {"X-Source": "contract"},
            "pollIntervalSeconds": 120,
        },
    )
    assert created.status_code == 200
    created_body = created.json()
    assert_keys(
        created_body,
        [*HTTP_SOURCE_INFO_KEYS, "status", "errorMessage"],
        "AddHttpSourceResponse",
    )
    assert created_body["bearerToken"] == MASKED_SECRET
    assert created_body["status"] == "connected"
    account_id = created_body["account"]["id"]

    listed = await client.get("/api/v1/accounts/http")
    assert listed.status_code == 200
    item = next(source for source in listed.json() if source["account"]["id"] == account_id)
    assert_keys(item, HTTP_SOURCE_INFO_KEYS, "HttpSourceInfo")
    assert item["bearerToken"] == MASKED_SECRET
    assert item["headers"] == {"X-Source": "contract"}

    patched = await client.patch(
        f"/api/v1/accounts/http/{account_id}",
        json={
            "method": "POST",
            "bodyType": "json",
            "body": '{"probe": true}',
            "bearerToken": MASKED_SECRET,
        },
    )
    assert patched.status_code == 200
    patched_body = patched.json()
    assert_keys(
        patched_body,
        [*HTTP_SOURCE_INFO_KEYS, "status", "errorMessage"],
        "AddHttpSourceResponse",
    )
    assert patched_body["method"] == "POST"
    assert patched_body["bearerToken"] == MASKED_SECRET


async def test_create_mqtt_broker_response_shape(client, app, monkeypatch):
    _stub_collector(app, monkeypatch)
    resp = await client.post(
        "/api/v1/accounts/mqtt",
        json={"brokerUrl": "mqtt://localhost:1883", "topics": ["news/#"]},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert_keys(body, ["account", "status", "errorMessage"], "AddMqttBrokerResponse")
    assert body["status"] == "connected"
    assert body["errorMessage"] is None


async def test_create_telegram_account_response_shape(client, app):
    """Without a collector the login fails, but the response shape must hold."""
    resp = await client.post(
        "/api/v1/accounts/telegram",
        json={"apiId": 111, "apiHash": "abc", "phone": "+886900000000"},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert_keys(body, ["nextStep", "account"], "AddAccountResponse")
    assert body["account"]["id"]
    assert body["nextStep"] == "error"  # no collector in tests
    stored = await app.state.db.fetch_value(
        "SELECT credentials FROM accounts WHERE id = ?",
        (body["account"]["id"],),
    )
    assert str(stored).startswith("enc:v1:")
    assert "abc" not in str(stored)


async def test_create_telegram_qr_account_response_shape(client, app):
    """Without a collector QR start fails, but the response shape must hold."""
    resp = await client.post(
        "/api/v1/accounts/telegram/qr",
        json={"apiId": 111, "apiHash": "abc"},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert_keys(body, ["nextStep", "account"], "AddAccountResponse")
    assert body["account"]["id"]
    assert body["account"]["name"] == "Telegram"
    assert body["nextStep"] == "error"


async def test_telegram_qr_wait_and_success_shape(client, app):
    class _QrCollector:
        async def start_telegram_qr_login(self, account_id: str, api_id: int, api_hash: str) -> dict:
            assert api_id == 222
            assert api_hash == "hash"
            return {
                "next_step": "qr_required",
                "qr_url": "tg://login?token=abc",
                "qr_expires_at": "2026-07-26T12:00:00Z",
            }

        async def wait_telegram_qr_login(self, account_id: str, timeout: float | None = None) -> dict:
            assert timeout == 20
            return {"next_step": "connected"}

    app.state.collector = _QrCollector()
    create_resp = await client.post(
        "/api/v1/accounts/telegram/qr",
        json={"apiId": 222, "apiHash": "hash"},
    )
    assert create_resp.status_code == 200
    create_body = create_resp.json()
    assert create_body["nextStep"] == "qr_required"
    assert create_body["pendingLoginStage"] == "qr_required"
    assert create_body["qrUrl"] == "tg://login?token=abc"
    assert create_body["qrExpiresAt"] == "2026-07-26T12:00:00Z"
    account_id = create_body["account"]["id"]

    wait_resp = await client.post(
        f"/api/v1/accounts/telegram/{account_id}/qr-wait",
        json={"timeoutSeconds": 20},
    )
    assert wait_resp.status_code == 200
    wait_body = wait_resp.json()
    assert wait_body["nextStep"] == "connected"
    assert wait_body["account"]["id"] == account_id


async def test_delete_account(client):
    resp = await client.delete(f"/api/v1/accounts/{seed.MQTT_ACCOUNT}")
    assert resp.status_code == 204
