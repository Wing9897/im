"""Contract: RRULE expansion via query helpers (retired HTTP occurrences)."""
...
from server.calendar.query_fetch import expand_active_calendar_occurrences
...
async def _expand(db, range_start: str, range_end: str, **kwargs):
    start = parse_iso(range_start)
    end = parse_iso(range_end, end_of_day=True)
    assert start is not None and end is not None
    return await expand_active_calendar_occurrences(db, start, end, **kwargs)


async def test_calendar_occurrences(app):
    body = await _expand(
        app.state.db,
    ...
    for occurrence in body:
        assert set(occurrence) == _EXPAND_KEYS
    assert body[0]["startTime"] == "2026-07-06T10:00:00Z"


async def test_calendar_includes_endpoints_and_skips_invalid_or_inactive_persisted_tasks(app, caplog):
...
async def test_calendar_persisted_mixture_preserves_allocation_contract_and_final_order(app, caplog):
    """The persisted expand path preserves skip, budget, boundary, and wire contracts."""
    ...
    body = await _expand(db, wire(day1_start), wire(day2_start))

    assert len(body) == 1000
    assert all(set(item) == _EXPAND_KEYS for item in body)
