...
_NOTIFY_CHECK = re.compile(
    r"notify_pref\s+TEXT\s+NOT\s+NULL\s+DEFAULT\s+'follow'\s+"
...
def _ddl_notify_values(ddl: str, label: str) -> frozenset[str]:
    ...
    values = {frozenset(re.findall(r"'([^']+)'", group)) for group in matches}
...
def test_worksets_notify_enabled_default_on() -> None:
    assert "notify_enabled INTEGER NOT NULL DEFAULT 1" in tasks_ddl.DDL
    assert "external_enabled INTEGER NOT NULL DEFAULT 1" in tasks_ddl.DDL
