...
def test_production_registry_is_stamp_2() -> None:
    assert tuple(step.target for step in SCHEMA_MIGRATIONS) == (2,)
...
@pytest.mark.asyncio
async def test_live_stamp_1_walks_to_2_and_keeps_rows(tmp_path) -> None:
    ...
    assert backups, "expected a pre-stamp-2 backup beside the database"
    assert any(".pre-stamp-2." in backup.name for backup in backups)
