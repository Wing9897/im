...
import { resetCalendarShareRateLimitForTests } from "../domain/calendarShare/calendarShareRateLimit.testing";
vi.mock("./client", () => ({
  apiClient: {
...
describe("calendarShare publish rate limit", () => {
  ...
  it("manual sync consumes the publish family budget", async () => {
    ...
    const { consumeCalendarShareRateLimit, CalendarShareRateLimitError } =
    ...
    await syncCalendarSharePublish(row);
    expect(() => consumeCalendarShareRateLimit("publish", 1_000)).toThrow(
  ...
  it("patches household auto-sync on /publish/auto-sync, not a workset path", async () => {
    ...
    const { patchCalendarSharePublishAutoSync } =
      await import("./calendarShare");
