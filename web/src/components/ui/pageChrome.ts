 ...
 * Inset rounded glass strip (``mx-page-x mt-md rounded-xl``); inner row capped
 * at max-w-[1280px]. Never use max-w-3xl. Not OpsControlBar — Settings/Ops
 * chrome stay separate.
 *
...
/**
 * Outer sticky band — inset glass strip (rounded + border), not a flush edge rail.
 * ``mx-page-x mt-md`` keeps the bar off the canvas edge (Timeline / Assistant language).
 */
export const pageChromeOuterClass =
  "im-surface-chrome sticky top-0 z-[100] mx-page-x mt-md shrink-0 rounded-xl border border-surface-border shadow-[var(--shadow-sm)] max-[780px]:mx-sm";
...
export const pageChromeInnerClass =
  `mx-auto flex w-full min-w-0 ${pageChromeMaxWidthClass} flex-wrap items-center gap-sm px-md py-sm`;
