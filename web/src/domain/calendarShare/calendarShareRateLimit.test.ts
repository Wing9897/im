...
describe("calendarShareRateLimit", () => {
  it("allows a subscribe then blocks the next one in the same window", () => {
    ...
    expect(() => consumeCalendarShareRateLimit("subscribe", 1_000)).toThrow(CalendarShareRateLimitError);
    expect(() => consumeCalendarShareRateLimit("subscribe", 1_000)).toThrowError("RATE_LIMITED");
  });
  ...
  it("allows two searches in the same window then blocks the third", () => {
    ...
    expect(() => consumeCalendarShareRateLimit("search", 1_200)).toThrow(CalendarShareRateLimitError);
  });

  it("refills after the window elapses", () => {
    resetCalendarShareRateLimitForTests();
    consumeCalendarShareRateLimit("subscribe", 1_000);
    consumeCalendarShareRateLimit("subscribe", 1_000 + CALENDAR_SHARE_RATE_LIMIT_WINDOW_MS);
  });
});
