...
import { getGeneralWorksetLabel } from "../../timeline/userEvents";
...
describe("notify scanner", () => {
  ...
  describe("lead calc / speak text", () => {
    ...
    it("computes fetch range with look-back and maxLead + buffer", () => {
      ...
      expect(getMaxLeadMinutes([30, 90])).toBe(90);
      expect(getMaxLeadMinutes([])).toBe(0);
    });
  });

  describe("filter by notify prefs", () => {
    const worksetNotifyById = new Map<string, boolean>([
      [SYSTEM_WORKSET_ID, true],
      ["ws-muted", false],
    ]);

    it("keeps follow-default on an enabled workset; drops muted follow and entity off", () => {
      const items = [
        makeEvent({
          id: "follow-on",
          kind: "user",
    ...
    it("mutes everything when global is off, including follow-on rows", () => {
      const items = [
        makeEvent({
          id: "follow",
          kind: "user",
  ...
  describe("collectDueReminders", () => {
    ...
    it("fires a custom 30-minute lead offset", () => {
      const startTime = "2026-07-19T10:00:00.000Z";
      const startMs = Date.parse(startTime);
      const nowMs = startMs - 30 * 60_000;
      const due = collectDueReminders({
        events: [makeEvent({ startTime })],
        leadOffsetsMinutes: [30],
        nowMs,
        firedKeys: new Set(),
      });
      expect(due).toHaveLength(1);
      expect(due[0]?.leadOffsetMinutes).toBe(30);
      expect(due[0]?.dedupeKey).toBe(buildDedupeKey("evt-1", 30, startTime));
      expect(due[0]?.speakText).toContain("約30分鐘");
    });
  });
