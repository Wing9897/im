...
import { useTaskForm, DEFAULT_FORM_STATE } from "./useTaskForm";
import type { TaskFormState, UseTaskFormReturn } from "./useTaskForm";
...
let latest: UseTaskFormReturn;

function Harness({ isSaving }: { isSaving?: boolean }) {
  latest = useTaskForm({ isSaving });
  return null;
}
...
describe("useTaskForm", () => {
  ...
  describe("canSave", () => {
    ...
    it("returns true in recurring mode when name and rrule are set", () => {
      renderHarness();
      act(() => {
