import { ONE_HOUR } from "./mapViewHelpers";
...
export function draw(
  ...
  lx: number, rx: number, drag: HitZone | null,
  ticks: number[], viewStart: number, viewSpan: number,
  dataMinX: number, dataMaxX: number,
): void {
  ...
  ctx.beginPath(); ctx.roundRect(0, TRACK_Y, w, TRACK_H, 7); ctx.fill();
  ...
  if (dataMaxX > dataMinX) {
    ...
    if (dRx > dLx) {
      ...
      ctx.beginPath(); ctx.roundRect(dLx, TRACK_Y, dRx - dLx, TRACK_H, 4); ctx.fill();
    }
  }
  ...
  for (const ts of ticks) {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(tx, TRACK_Y); ctx.lineTo(tx, TRACK_Y + TRACK_H); ctx.stroke();
  }
  ...
  if (selR > selL) {
    ...
    ctx.beginPath(); ctx.roundRect(selL, TRACK_Y, selR - selL, TRACK_H, 4); ctx.fill();
  }
  ...
  const drawH = (cx: number, active: boolean) => {
    ...
    ctx.beginPath(); ctx.roundRect(cx - HANDLE_W / 2, HANDLE_Y, HANDLE_W, HANDLE_H, 3); ctx.fill();
    ctx.shadowBlur = 0;
    ctx.strokeStyle = cssVar("--surface-base", "#1e1e2e"); ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(cx - 2, HANDLE_Y + 10); ctx.lineTo(cx - 2, HANDLE_Y + 20);
    ctx.moveTo(cx + 2, HANDLE_Y + 10); ctx.lineTo(cx + 2, HANDLE_Y + 20);
    ctx.stroke();
  };
  drawH(lx, drag === "left");
