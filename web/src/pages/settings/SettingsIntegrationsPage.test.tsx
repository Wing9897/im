...
const { settingsState, updateSettings, handleSave } = vi.hoisted(() => {
  ...
  const updateSettings = vi.fn((key: keyof typeof settingsState, value: unknown) => {
...
describe("SettingsIntegrationsPage", () => {
  ...
  function tabButton(label: string): HTMLButtonElement | undefined {
    return Array.from(container.querySelectorAll('button[role="tab"]')).find((btn) =>
  ...
  it("defaults to Webhook with origin URL, key link, and collapsed examples", () => {
    renderPage();
    expect(container.querySelector('[data-testid="api-docs-base-url"]')?.textContent).toBe(
      "http://127.0.0.1:18820",
    );
    expect(container.querySelector('[data-testid="integrations-webhook-link-keys"]')?.getAttribute("href")).toBe(
      "/account/keys",
    );
    ...
    expect(container.querySelector('[data-testid="access-key-create"]')).toBeNull();
    expect(container.textContent).not.toContain("唯讀");
  });

  it("shows Webhook, A2A, Calendar link, and MCP as in-page pills", () => {
    renderPage();
    const pills = container.querySelector('[data-testid="integrations-subtabs"]');
    ...
    expect(pills?.querySelector('[role="tablist"]')?.className).toContain("inline-flex");
  ...
  it("opens the webhook example when the collapsed section is expanded", () => {
    ...
    expect(container.querySelector('[data-testid="api-docs-example"]')).not.toBeNull();
  });
  ...
  it("switches to A2A and MCP sub-pages", () => {
    ...
    expect(container.querySelector('[data-testid="integrations-a2a-docs"]')).not.toBeNull();
    expect(container.querySelector('[data-testid="integrations-a2a-link-keys"]')?.getAttribute("href")).toBe(
      "/account/keys",
    );
    ...
    expect(container.querySelector('[data-testid="a2a-master-switch"]')).not.toBeNull();
    expect(container.querySelector('[data-testid="mcp-master-switch"]')).toBeNull();
  ...
  it("binds the same capability toggles on A2A and MCP and persists a toggle across tabs", () => {
    ...
    expect(container.querySelector('[data-testid="mcp-cap-mcpCapItemsWrite"]')?.getAttribute("aria-checked")).toBe(
      "false",
    );
    expect(container.querySelector('[data-testid="mcp-master-switch"]')).toBeNull();
    expect(container.querySelector('[data-testid="a2a-master-switch"]')).not.toBeNull();
  ...
  it("does not show capability toggles on Webhook or Calendar link tabs", () => {
    renderPage();
    expect(container.querySelector('[data-testid="mcp-capability-toggles"]')).toBeNull();
    expect(container.querySelector('[data-testid="mcp-workset-hub"]')).toBeNull();
  ...
  it("opens MCP from ?tab=mcp", () => {
    renderPage("/settings/integrations?tab=mcp");
    expect(container.querySelector('[data-testid="mcp-endpoint-url"]')?.textContent).toBe(
      "http://127.0.0.1:18820/api/v1/mcp",
    );
    expect(container.querySelector('[data-testid="mcp-capability-toggles"]')).not.toBeNull();
  });
  ...
  it("opens deeplink from ?tab=deeplink", () => {
    ...
    expect(container.querySelector('[data-testid="integrations-deeplink-url"]')?.textContent).toContain(
      "intelligencemonitor://",
    );
    expect(container.querySelector('[data-testid="integrations-deeplink-docs"]')?.textContent).toContain(
      "日曆連結",
    );
    expect(container.querySelector('[data-testid="integrations-deeplink-docs"]')?.textContent).toContain(
      "僅 Desktop",
  ...
  it("toggles A2A master enable on the A2A tab without touching MCP enable", async () => {
    ...
    const master = container.querySelector<HTMLButtonElement>('[data-testid="a2a-master-switch"]');
    ...
    expect(container.querySelector('[data-testid="mcp-master-switch"]')).toBeNull();
    expect(container.querySelector('[data-testid="a2a-caps-inactive-hint"]')).toBeNull();
    ...
    expect(updateSettings).not.toHaveBeenCalledWith("mcpEnabled", expect.anything());
  });

  it("shows the A2A inactive hint when A2A is off and keeps MCP enable independent", () => {
    settingsState.a2aEnabled = false;
    renderPage("/settings/integrations?tab=a2a");
    expect(container.querySelector('[data-testid="a2a-caps-inactive-hint"]')?.textContent).toContain(
      "A2A 關閉",
    );
    expect(container.querySelector('[data-testid="mcp-caps-inactive-hint"]')).toBeNull();
    expect(container.querySelector('[data-testid="a2a-master-switch"]')?.getAttribute("aria-checked")).toBe(
      "false",
    );
    ...
    expect(container.querySelector('[data-testid="mcp-master-switch"]')?.getAttribute("aria-checked")).toBe(
      "true",
    );
    expect(container.querySelector('[data-testid="a2a-master-switch"]')).toBeNull();
  });
