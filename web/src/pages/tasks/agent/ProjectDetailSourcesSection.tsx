...
interface ProjectDetailSourcesSectionProps {
  channelLabels: string[];
}

export function ProjectDetailSourcesSection({ channelLabels }: ProjectDetailSourcesSectionProps) {
  ...
  return (
    <>
      <PanelSection
        title={t("tasks.projectDetail.sourcesTitle")}
        showCount
        ...
        headerActions={
          channelLabels.length > SOURCE_PREVIEW ? (
            ...
            >
              {t("tasks.projectDetail.sourcesViewAll", { count: channelLabels.length })}
            </Button>
          ) : null
        }
      >
      ...
      {sourcesOpen ? (
        <DetailPresentationShell
          ...
          aria-label={t("tasks.projectDetail.sourcesDialogTitle")}
        >
          <header className={`${taskDetailHeaderClass} shrink-0`}>
            <h2 className={taskDetailTitleClass}>
              {t("tasks.projectDetail.sourcesDialogTitle")}
            </h2>
