...
import {
  buildProjectTickSummaryView,
  deriveTickOutcome,
  tickOutcomeTone,
  type TickOutcome,
} from "./projectTickSummary";

interface ProjectDetailTickSectionProps {
  tickStatus: ProjectTickStatus | null;
  activitySpan: TaskActivitySpan | null;
  spanLoading: boolean;
}

export function ProjectDetailTickSection({
  tickStatus,
  activitySpan,
  spanLoading,
}: ProjectDetailTickSectionProps) {
  ...
  const tickView = useMemo(
    () => buildProjectTickSummaryView(tickStatus, activitySpan),
      ...
      outcome === "success"
        ...
        : outcome === "skipped"
          ...
          : outcome === "running"
            ? "tickOutcomeRunning"
            : "tickOutcomeError";
    return t(`tasks.projectDetail.${key}`);
  };

  return (
    <>
      <PanelSection title={t("tasks.projectDetail.tickTitle")}>
          ...
          <div className="flex min-w-0 flex-col gap-sm" data-testid="project-detail-tick">
            <dl className="grid gap-sm text-body sm:grid-cols-2">
              <div className="min-w-0">
                <dt className={captionClass}>{t("tasks.projectDetail.tickLastEnd")}</dt>
              ...
              <div className="min-w-0">
                <dt className={captionClass}>{t("tasks.projectDetail.tickCompletedCount")}</dt>
                <dd className="text-text-primary tabular-nums">
                  {activitySpan?.completedBatchCount ?? 0}
                </dd>
              </div>
              <div className="min-w-0">
                <dt className={captionClass}>{t("tasks.projectDetail.tickOutcome")}</dt>
              ...
              <div className="min-w-0">
                <dt className={captionClass}>{t("tasks.projectDetail.tickMessageCount")}</dt>
                <dd
              ...
              <div className="min-w-0">
                <dt className={captionClass}>{t("tasks.projectDetail.tickPendingCursor")}</dt>
                <dd
                  className="text-text-primary tabular-nums"
              ...
              <div className="min-w-0">
                <dt className={captionClass}>{t("tasks.projectDetail.tickCursorAt")}</dt>
                <dd className="break-words text-text-primary">
            ...
            {lastTickError ? (
              ...
              >
                <span className="font-medium">{t("tasks.projectDetail.tickError")}: </span>
            ...
            <div>
              ...
              >
                {tickExtrasOpen
                  ? t("tasks.projectDetail.hideTickExtras")
                  : t("tasks.projectDetail.showTickExtras")}
              </Button>
              {tickExtrasOpen ? (
                <div className="mt-sm flex min-w-0 flex-col gap-sm">
                  <div data-testid="project-detail-tick-message">
                    <div className={captionClass}>{t("tasks.projectDetail.tickMessage")}</div>
                    {agentMessage ? (
                      ...
                      <p className={`mt-0.5 ${captionClass}`}>
                        {t("tasks.projectDetail.tickNoMessage")}
                      </p>
                    )}
                  </div>
                  <div data-testid="project-detail-tick-tools">
                    {toolCalls.length > 0 ? (
                      ...
                      <>
                        <div className={captionClass}>{t("tasks.projectDetail.tickTools")}</div>
                        <p className={`mt-0.5 ${captionClass}`}>
                          {t("tasks.projectDetail.tickNoTools")}
                        </p>
                      </>
                    )}
                  </div>
      ...
      <PanelSection
        title={t("tasks.projectDetail.tickLogTitle")}
      ...
      >
        {spanLoading && tickLog.length === 0 ? (
          ...
          <p className={captionClass} data-testid="project-detail-tick-log-empty">
            {inFlight
              ? t("tasks.projectDetail.tickLogEmptyInFlight")
              : t("tasks.projectDetail.tickLogEmpty")}
          </p>
        ) : (
            ...
            {tickLog.map((entry) => {
              ...
              return (
                <li
                  ...
                  data-testid={`project-tick-log-${entry.batchId}`}
                >
