...
export function deriveTickOutcome(
  entry: Pick<ProjectTickLogEntry, "outcome" | "errorMessage" | "agentMessage"> | null | undefined,
...
export interface ProjectTickSummaryView {
  latestTick: ProjectTickLogEntry | null;
  inFlight: ProjectTickInFlight | null;
  outcome: TickOutcome | null;
  errorMessage: string | null;
  messageCount: number | null;
  pendingSinceCursor: number | null;
  tickLog: ProjectTickLogEntry[];
  toolCalls: NonNullable<ProjectTickLogEntry["toolCalls"]>;
  agentMessage: string;
}

export function buildProjectTickSummaryView(
  tickStatus: ProjectTickStatus | null,
  activitySpan: TaskActivitySpan | null,
): ProjectTickSummaryView {
